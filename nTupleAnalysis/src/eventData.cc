...
void eventData::buildViews(){
  ...
  views[0]->SRvsSB_pull[lowBinEdge_ind]  = getSRvsSB_Pull(views[0]->m4j, views[0]->leadSt->m, views[0]->sublSt->m,lowBinEdge_ind);
  views[1]->SRvsSB_pull[lowBinEdge_ind]  = getSRvsSB_Pull(views[1]->m4j, views[1]->leadSt->m, views[1]->sublSt->m,lowBinEdge_ind);
  views[2]->SRvsSB_pull[lowBinEdge_ind] = getSRvsSB_Pull(views[2]->m4j, views[2]->leadSt->m, views[2]->sublSt->m,lowBinEdge_ind);
  }
  // Ends here
...
float eventData::getSRvsSB_Pull (float m4j, float leadSt_m, float sublSt_m, int lowBinEdge_ind)
{
  // std::map<int, float> pull_for_bin;
  // for (int lowBinEdge_ind = 0; lowBinEdge_ind < 10; lowBinEdge_ind++) {
    // Get The right 2d Histogram
    TH2F* thisM4jHist = getSRvsSB_PullHist(lowBinEdge_ind);
    
    // Returns the corresponding pull value for an events that lies in the bin of the pull 2D histogram
    if(thisM4jHist)
      return thisM4jHist->GetBinContent(thisM4jHist->FindBin(leadSt_m, sublSt_m)); //pull_for_bin[lowBinEdge_ind] =
...
TH2F* eventData::getSRvsSB_PullHist(int lowBinEdge_ind){

  float m4jBinIndex = 300 + lowBinEdge_ind * 50; // m4jBinIndex holds mass  
